...
class Monkey:
    def __init__(self,starting_items: list[int], operation_str: str, divisible: int,if_true: int,if_false: int) -> None:
    ...
    def take_turn(self):
        ...
        while(len(self.items) > 0):
            ...
            throws.append((item,self.throw_to(item)))
        if verbose:
            print()
        return throws
...
if __name__ == "__main__":
    ...
    while(i < len(all_lines)):
        ...
        if all_lines[i][0] == 'Monkey':
            ...
            monkeys.append(Monkey(items,op_str,divisible,if_true,if_false))
        i += 1
